...
export class BlockPolling {
  ...
  constructor(provider: ethers.providers.BaseProvider, eventFetcher: EventFetcher, options: Options = {}) {
  ...
  private async _poll(fromBlock?: number) {
    try {
      const latestBlock = await this._provider.getBlockNumber() - this._confirmations;
