...
handler
	.get(async (req: ApiRequest, res: ApiResponse) => {
		...
		if (existingConvId) {
			...
			} else if (sConvSel.data !== null && sConvSel.data.length > 0) {
	...
	.post(async (req: ApiRequest, res: ApiResponse) => {
		...
		if (sIns.error !== null && sIns.status !== 406) {
			handleException(
				ono(
					sIns.error,
		...
		if (sIns.data === null || isEmpty(sIns.data)) {
			return res.status(400).json({
				success: false
			});
		}
